...
static inline Std_ReturnType Pwm_ValidateParamsPhaseShift(uint16    PhaseShift,
                                                          uint8     ServiceId);
#endif
                                                           ...
                                                           uint8           ServiceId)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
#if ((PWM_SET_PHASE_SHIFT_API == STD_ON) || (PWM_SET_PHASE_SHIFT_NO_UPDATE_API == STD_ON) ||(PWM_SET_DUTY_PHASE_SHIFT_API == STD_ON))
                if ((PWM_SETPHASESHIFT_ID == ServiceId) || (PWM_SETPHASESHIFTNOUPDATE_ID == ServiceId) || (PWM_SETDUTYPHASESHIFT_ID == ServiceId))
                {
...
#if ((PWM_SET_PERIOD_AND_DUTY_NO_UPDATE_API == STD_ON) || (PWM_SET_DUTY_CYCLE_NO_UPDATE_API == STD_ON) || (PWM_SET_PHASE_SHIFT_NO_UPDATE_API == STD_ON) || (PWM_SYNC_UPDATE_API == STD_ON))
                if ((PWM_SETPERIODANDDUTY_NO_UPDATE_ID == ServiceId) || (PWM_SETDUTYCYCLE_NO_UPDATE_ID == ServiceId) || (PWM_SETPHASESHIFTNOUPDATE_ID == ServiceId) || (PWM_SYNCUPDATE_ID == ServiceId))
                {
...
#if ((PWM_SET_PHASE_SHIFT_API == STD_ON) || (PWM_SET_PHASE_SHIFT_NO_UPDATE_API == STD_ON))
/**
* @brief        Pwm_ValidateParamsPhaseShift
...
* @param[in]    ServiceId           Id of the service calling this function
*
* @return       Std_ReturnType      Validity of the phase shift parameter
* @retval       E_OK                Pwm_SetPhaseShift is supported this channel
* @retval       E_NOT_OK            Pwm_SetPhaseShift is not supported this channel
*
*/
static inline Std_ReturnType Pwm_ValidateParamsPhaseShift(uint16 PhaseShift,
                                                          uint8  ServiceId)
...
#endif /* PWM_SET_PHASE_SHIFT_API || PWM_SET_PHASE_SHIFT_NO_UPDATE_API */
...
*
*/
const Pwm_IpConfigType * Pwm_GetIpConfigPtr(void)
{
    ...
    /* If global config pointer is NULL_PTR, return NULL_PTR pointer */
    if (NULL_PTR == Pwm_pConfig[CoreId])
    {
        IpConfig = NULL_PTR;
    }
    else
    {
        IpConfig = &Pwm_pConfig[CoreId]->IpConfig;
    }

    return IpConfig;
}
#endif /* PWM_FAULT_SUPPORTED || PWM_RELOAD_NOTIF_SUPPORTED */
...
*
*/
void Pwm_SetPhaseShift_NoUpdate(Pwm_ChannelType ChannelNumber,
                                Pwm_PeriodType  Period,
                                uint16          PhaseShift)
{
    uint32          CoreId;
...
#if (PWM_VALIDATE_CHANNEL_CONFIG_CALL == STD_ON)
    if ((Std_ReturnType)E_OK == Pwm_ValidateChannelConfigCall(CoreId, ChannelNumber, PWM_SETPHASESHIFTNOUPDATE_ID))
    {
#endif
        PartitionChannelIdx = Pwm_GetPartitionChannelIdx(CoreId, ChannelNumber);

#if (PWM_PARAM_CHECK == STD_ON)
        if ((Std_ReturnType)E_OK == Pwm_ValidateParamsPeriodClass(CoreId, PartitionChannelIdx, (*Pwm_aState[CoreId].PwmChannelConfigs)[PartitionChannelIdx].PwmChannelClass, Period, PWM_SETPHASESHIFTNOUPDATE_ID))
        {
            if ((Std_ReturnType)E_OK == Pwm_ValidateParamsPhaseShift(PhaseShift, PWM_SETPHASESHIFTNOUPDATE_ID))
            {
#endif

                /* All validations passed. Here starts the actual functional code of the function */
                Pwm_Ipw_SetPhaseShift_NoUpdate(Period, PhaseShift, &((*Pwm_aState[CoreId].PwmChannelConfigs)[PartitionChannelIdx].IpwChannelCfg));
...
*
*/
void Pwm_SetPhaseShift(Pwm_ChannelType  ChannelNumber,
                       Pwm_PeriodType   Period,
                       uint16           PhaseShift)
{
    uint32          CoreId;
...
#if (PWM_VALIDATE_CHANNEL_CONFIG_CALL == STD_ON)
    if ((Std_ReturnType)E_OK == Pwm_ValidateChannelConfigCall(CoreId, ChannelNumber, PWM_SETPHASESHIFT_ID))
    {
#endif
        PartitionChannelIdx = Pwm_GetPartitionChannelIdx(CoreId, ChannelNumber);

#if (PWM_PARAM_CHECK == STD_ON)
        if ((Std_ReturnType)E_OK == Pwm_ValidateParamsPeriodClass(CoreId, PartitionChannelIdx, (*Pwm_aState[CoreId].PwmChannelConfigs)[PartitionChannelIdx].PwmChannelClass, Period, PWM_SETPHASESHIFT_ID))
        {
            if ((Std_ReturnType)E_OK == Pwm_ValidateParamsPhaseShift(PhaseShift, PWM_SETPHASESHIFT_ID))
            {
#endif

                /* All validations passed. Here starts the actual functional code of the function */
                Pwm_Ipw_SetPhaseShift(Period, PhaseShift,&((*Pwm_aState[CoreId].PwmChannelConfigs)[PartitionChannelIdx].IpwChannelCfg));
