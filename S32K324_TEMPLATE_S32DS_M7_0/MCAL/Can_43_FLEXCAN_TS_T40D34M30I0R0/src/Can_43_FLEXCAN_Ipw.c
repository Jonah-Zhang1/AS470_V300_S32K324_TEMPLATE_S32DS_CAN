...
#if (CAN_43_FLEXCAN_ENHANCED_FIFO_ENABLED == STD_ON)
        if ((CAN_RX_ENHANCED_FIFO == Can_pHwObject->Can_eReceiveType) && ((Can_pHwObject->Can_u8HwFilterCount) != 0U))
        {
...
{
#if (CAN_43_FLEXCAN_ENHANCED_FIFO_ENABLED == STD_ON)
    if (CAN_RX_ENHANCED_FIFO == Can_pHwObjectConfig->Can_eReceiveType)
    {
    #if (FLEXCAN_IP_FEATURE_HAS_DMA_ENABLE == STD_ON)
        if (FLEXCAN_RXFIFO_USING_DMA == Can_pControllerConfig->HwChannelIpConfig->pFlexcanIpHwConfig->transfer_type)
        {
